...
class StringRow {
	...
	getTension() {
		...
		let stringType = gauge[gauge.length - 1]
		let coefs = [];
		if (stringType == 'P')
			coefs = [ -.000176520934, .0840206843, -16.01839853, 1656.456428,
				  -96731.24564, 3248319.241, -58293798.41, 432468921.1 ]
		else if (stringType == 'W')
			coefs = [ -.002123403683, .4863557681, -46.19498733, 2403.599196,
				  -74026.84724, 1389623.565, -15576312.23, 95696503.28, -247760614.2 ];
		else if (stringType == 'H')
			coefs = [ 5.0562e-05, -0.0064271, 0.580635, -11.0332, 166.137,
				  -1404.43, 6595.03, -16005.8, 15625.9 ];
